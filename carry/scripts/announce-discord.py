...
# 3560 is the Bounty Teleportation page. The environment still wins, so a one-off
# post to somewhere else does not mean editing this.
NEXUS_FILES = (os.environ.get("NEXUS_FILES_URL", "").strip() or
               "https://www.nexusmods.com/crimsondesert/mods/3560?tab=files")
GITHUB_RELEASE = "https://github.com/shin2344234/bounty-teleportation/releases/tag/v%s"
...
def post(token, channel, content):
    ...
    req = urllib.request.Request(url, data=json.dumps({"content": content}).encode("utf-8"),
                                 headers={"Content-Type": "application/json",
                                          ...
                                          "User-Agent": "BountyTeleportation-announce"})
