<#
.SYNOPSIS
    Sign BountyTeleportation.asi with Azure Trusted Signing, then verify it.
...
.DESCRIPTION
    ...
    under Microsoft's identity-verified chain, the same one the other mods
    ship with.
    ...
    Only the shipping plugin is ever signed. Bounty Probe, the research
    plugin in mod\, is not published and does not get a signature.

.EXAMPLE
    .\sign.ps1                       signs carry\dist\BountyTeleportation.asi
...
if (-not $Path) { $Path = Join-Path $repo 'carry\dist\BountyTeleportation.asi' }
...
foreach ($f in @($metadata, $dlib, $signtool)) {
    if (-not (Test-Path -LiteralPath $f)) {
        throw "Missing $f. private\tools is a copy of vpk's vendor\signing folder, as used by the other mods."
    }
}
