...
def release_files(version):
    return [
        ("BountyTeleportation-%s-DMM.zip" % version, "the DMM archive"),
        ("BountyTeleportation-%s.zip" % version, "the full archive"),
        ("BountyTeleportation.asi", "the plugin"),
    ]
...
def version_from_header():
    ...
    m = re.search(r'#define\s+BT_VERSION\s+"([^"]+)"', text)
    if not m:
        raise SystemExit("no BT_VERSION in %s" % h)
    return m.group(1)
...
def main():
    ...
    plugin = next((r for r in rows if r[0] == "BountyTeleportation.asi" and r[3]), None)
