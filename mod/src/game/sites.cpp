...
namespace
{
    constexpr int kMaxSites   = 128;  // session nine's ini is 70 sites plus the automatic ones
    ...
    struct Site
    {
        ...
        bool      onErr = false;      // say nothing unless the out pointer comes back non-zero
        bool      onNull = false;     // say nothing unless the function returns zero
        char      only[48] = "";      // per-call lines only when an argument's class name contains this
        uint8_t   markArgs = 0;       // mark=: bit k remembers argument k+1 of every wanted call
        bool      onlyMarked = false; // per-call lines only when an argument is marked, or belongs to one
        unsigned  marksBytes = 0;     // marks=: dump every marked object this many bytes after a written call
        // No budgets by default: every call is written in full. The options
        // that cap these still parse, for the tests and for a site someone
        // wants quiet, but nothing in the ini uses them.
        static constexpr LONG kUnlimited = 0x7FFF0000;
        volatile LONG dumpLeft = kUnlimited, linesLeft = kUnlimited, stackLeft = kUnlimited, errLinesLeft = kUnlimited;
        volatile LONG calls = 0, returns = 0, overridden = 0, errors = 0;
        volatile LONG64 lastRet = 0, lastErr = 0;
        LONG reportedCalls = 0, reportedErrors = 0; // mod thread only
    };
    ...
    volatile LONG64 g_marks[kMaxMarks] = {};
    ...
    // `a` and `caller` are only read back when a site logs on outcome: the
    // entry has said nothing by then, so the return has to carry it.
    struct RetEntry { int site; uint64_t ret; uint64_t* slot; LONG call; uint64_t outPtr;
                      uint64_t caller; uint64_t a[4]; bool shown; };
    ...
    {
        wchar_t w[48] = {};
        if (!bp::mem::ReadBytes(p, w, sizeof(wchar_t) * 47)) return false;
        size_t k = 0;
        for (; k < 47 && w[k]; ++k)
    ...
    void FmtArg(uint64_t v, char* out, size_t n)
    {
        ...
        if (!bp::mem::Plausible(p) || !bp::mem::Readable(p, 16))
        {
            if (bp::mem::InImage(p)) snprintf(out, n, "img+0x%llX", static_cast<unsigned long long>(bp::mem::Rva(p)));
            else snprintf(out, n, "0x%llX", static_cast<unsigned long long>(v));
            return;
        }
        const char* rt = bp::mem::RttiShort(p);
        if (rt) { snprintf(out, n, "0x%llX<%s>", static_cast<unsigned long long>(v), rt); return; }
        char text[64];
        if (bp::mem::ReadCString(p, text, sizeof text) && strlen(text) >= 3)
        {
    ...
    void WalkCaller(const char* name, uint64_t* retSlot, uint64_t realRet, int maxFrames)
    {
        ...
        for (int f = 0; f < maxFrames; ++f)
        {
            ...
            LOG("[site]     +0x%llX", static_cast<unsigned long long>(bp::mem::Rva(static_cast<uintptr_t>(ctx.Rip))));
            DWORD64 imgBase = 0;
            RUNTIME_FUNCTION* fe = RtlLookupFunctionEntry(ctx.Rip, &imgBase, nullptr);
            if (!fe)
            {
                uint64_t r = 0;
                if (!bp::mem::Read64(static_cast<uintptr_t>(ctx.Rsp), &r)) break;
                ctx.Rip = r; ctx.Rsp += 8;
                continue;
            }
            ...
            if (!ctx.Rip || !bp::mem::Readable(static_cast<uintptr_t>(ctx.Rsp), 8)) break;
        }
    }
    ...
    // Case-insensitive substring on the RTTI name of a pointer argument.
    bool ClassContains(uint64_t v, const char* needle)
    {
        const uintptr_t p = static_cast<uintptr_t>(v);
        if (!bp::mem::Plausible(p) || !bp::mem::Readable(p, 16)) return false;
        const char* rt = bp::mem::RttiShort(p);
        if (!rt) return false;
        const size_t n = strlen(needle);
        for (const char* q = rt; *q; ++q)
            if (!_strnicmp(q, needle, n)) return true;
        return false;
    }
    ...
    int __cdecl OnEnter(int i, uint64_t* frame, uint64_t* retSlot)
    {
        ...
        RetEntry* mine = nullptr;
        bool printed = false;

        // `only=`: a hot function whose interesting calls are the few with a
        // particular class among the arguments. The world load spent every
        // fm_* budget on actors that were not the one being carried.
        const uint64_t vals[6] = { frame[0], frame[1], frame[2], frame[3],
                                   bp::mem::Readable(reinterpret_cast<uintptr_t>(retSlot + 5), 8) ? retSlot[5] : 0,
                                   bp::mem::Readable(reinterpret_cast<uintptr_t>(retSlot + 6), 8) ? retSlot[6] : 0 };
        bool wanted = true;
        if (s.only[0])
        {
            wanted = false;
            for (unsigned k = 0; k < s.args && k < 6 && !wanted; ++k) wanted = ClassContains(vals[k], s.only);
        }
        // `mark=`: remember these arguments. `onlymarked=1`: speak only about
        // calls that carry a remembered object, or a piece of one. An
        // `onerr=1` site stays quiet about the rest and still reports their
        // errors at return.
        if (wanted && s.markArgs)
            for (unsigned k = 0; k < 6; ++k)
                if (s.markArgs & (1u << k)) bp::sites::Mark(vals[k], s.name);
        bool hit = false;
        if (s.onlyMarked)
            for (unsigned k = 0; k < s.args && k < 6 && !hit; ++k) hit = bp::sites::Interesting(vals[k]);
        const bool talk = wanted && (s.onlyMarked ? hit : (!s.onErr && !s.onNull));
        ...
        if (talk && InterlockedCompareExchange(&s.linesLeft, 0, 0) > 0)
        {
            ...
            else
            {
                ...
                LOG("[site] %s call %ld from +0x%llX"
                    ...
                    s.name, n, static_cast<unsigned long long>(bp::mem::Rva(caller)),
                    s.args > 0 ? " rcx=" : "", a[0],
                    s.args > 1 ? " rdx=" : "", a[1],
                    s.args > 2 ? " r8=" : "", a[2],
                    s.args > 3 ? " r9=" : "", a[3],
        ...
        // Dumps and the unwind follow the entry line: a call `only=` or
        // `onlymarked=` kept quiet gets neither.
        if (printed && InterlockedCompareExchange(&s.dumpLeft, 0, 0) > 0)
        {
            ...
            for (int k = 0; k < 3 && static_cast<unsigned>(k) < s.args; ++k)
            {
                ...
                if (bp::mem::Plausible(p) && bp::mem::Readable(p, s.objBytes))
                {
                    char tag[64];
                    snprintf(tag, sizeof tag, "%s %s", s.name, what[k]);
                    bp::dump::Object(tag, p, s.objBytes);
                }
            }
        }
        // `marks=`: the marked objects as they are at this moment, which for
        // the outlaw's move is the player's catch component.
        if (printed && s.marksBytes)
        {
            for (int k = 0; k < 32; ++k)
            {
                const uint64_t m = bp::sites::MarkAt(k);
                if (!m) break;
                const uintptr_t p = static_cast<uintptr_t>(m);
                if (!bp::mem::Readable(p, s.marksBytes)) continue;
                char tag[64];
                snprintf(tag, sizeof tag, "%s marked %d", s.name, k);
                bp::dump::Object(tag, p, s.marksBytes);
            }
        }
        if (printed && InterlockedCompareExchange(&s.stackLeft, 0, 0) > 0)
        {
            InterlockedDecrement(&s.stackLeft);
            WalkCaller(s.name, retSlot, caller, 24);
        }
        if (mine) mine->shown = printed;
        return skipNow ? 1 : 0;
    }
    ...
    uint64_t __cdecl OnLeave(int i, uint64_t* raxSlot)
    {
        ...
        LONG call = 0;
        uint64_t outPtr = 0;
        uint64_t callerSeen = 0;
        uint64_t argsSeen[4] = {};
        bool shownSeen = InterlockedCompareExchange(&s.linesLeft, 0, 0) > 0;
        if (ts)
        {
            ...
            if (k >= 0)
            {
                ...
                callerSeen = ts->e[k].caller;
                shownSeen = ts->e[k].shown;
                for (int q = 0; q < 4; ++q) argsSeen[q] = ts->e[k].a[q];
                ts->depth = k;
            }
        }
        ...
        // The out pointer is the error slot on every function here. Read it
        // whatever the budgets say: a count of refusals costs nothing and it
        // is the one number a session cannot do without.
        uint32_t outVal = 0;
        const bool haveOut = s.outArg && bp::mem::Plausible(static_cast<uintptr_t>(outPtr)) &&
                             bp::mem::Read32(static_cast<uintptr_t>(outPtr), &outVal);
        if (haveOut && outVal)
        {
            InterlockedIncrement(&s.errors);
            InterlockedExchange64(&s.lastErr, static_cast<LONG64>(outVal));
        }
        // A lookup that failed says so by returning nothing, and no error code
        // is written anywhere, so onerr would never see it.
        if (s.onNull)
        {
            if (*raxSlot == 0)
            {
                InterlockedIncrement(&s.errors);
                if (InterlockedCompareExchange(&s.errLinesLeft, 0, 0) > 0)
                {
                    InterlockedDecrement(&s.errLinesLeft);
                    char a[4][96];
                    for (int k = 0; k < 4; ++k)
                        FmtArg(k < static_cast<int>(s.args) ? argsSeen[k] : 0, a[k], sizeof a[k]);
                    LOG("[site] %s call %ld returned nothing, called from +0x%llX with rcx=%s rdx=%s r8=%s r9=%s",
                        s.name, call,
                        static_cast<unsigned long long>(bp::mem::Rva(static_cast<uintptr_t>(callerSeen))),
                        a[0], a[1], a[2], a[3]);
                    if (s.objBytes && bp::mem::Plausible(static_cast<uintptr_t>(argsSeen[0])))
                        bp::dump::Object(s.name, static_cast<uintptr_t>(argsSeen[0]), s.objBytes);
                    if (s.objBytes && bp::mem::Plausible(static_cast<uintptr_t>(argsSeen[1])))
                        bp::dump::Object(s.name, static_cast<uintptr_t>(argsSeen[1]), s.objBytes);
                }
            }
            return ret;
        }
        if (s.onErr && !shownSeen)
        {
            if (haveOut && outVal && InterlockedCompareExchange(&s.errLinesLeft, 0, 0) > 0)
            {
                InterlockedDecrement(&s.errLinesLeft);
                const char* en = bp::errors::Name(outVal);
                char a[4][96];
                for (int k = 0; k < 4; ++k)
                    FmtArg(k < static_cast<int>(s.args) ? argsSeen[k] : 0, a[k], sizeof a[k]);
                LOG("[site] %s call %ld returned %lu (0x%lX)%s%s, called from +0x%llX with rcx=%s rdx=%s "
                    "r8=%s r9=%s", s.name, call, static_cast<unsigned long>(outVal),
                    static_cast<unsigned long>(outVal), en ? " = " : "", en ? en : "",
                    static_cast<unsigned long long>(bp::mem::Rva(static_cast<uintptr_t>(callerSeen))),
                    a[0], a[1], a[2], a[3]);
                if (s.objBytes && bp::mem::Plausible(static_cast<uintptr_t>(argsSeen[0])))
                    bp::dump::Object(s.name, static_cast<uintptr_t>(argsSeen[0]), s.objBytes);
        ...
        // The return line follows the entry line: a call the entry did not
        // write (budget, or `only=` not matched) gets no return line either.
        const bool quiet = !shownSeen;
        if (!quiet)
        {
            char a[96];
            FmtArg(*raxSlot, a, sizeof a);
            LOG("[site] %s call %ld returned %s", s.name, call, a);
            if (s.outArg && bp::mem::Plausible(static_cast<uintptr_t>(outPtr)) &&
                bp::mem::Readable(static_cast<uintptr_t>(outPtr), 32))
            {
                uint32_t d = 0;
                bp::mem::Read32(static_cast<uintptr_t>(outPtr), &d);
                char q[4][96];
                for (int k = 0; k < 4; ++k)
                {
                    uint64_t v = 0;
                    bp::mem::Read64(static_cast<uintptr_t>(outPtr) + 8 * k, &v);
                    FmtArg(v, q[k], sizeof q[k]);
                }
                // An out pointer on one of these functions is nearly always the
                // eErr slot, so the u32 gets looked up by name. An unknown code
                // still prints, as a number.
                const char* en = bp::errors::Name(d);
                LOG("[site]   %s out arg%u -> u32 %lu (0x%lX)%s%s; qwords %s | %s | %s | %s", s.name, s.outArg,
                    static_cast<unsigned long>(d), static_cast<unsigned long>(d),
                    en ? " = " : "", en ? en : "", q[0], q[1], q[2], q[3]);
            }
            if (s.retObj && InterlockedCompareExchange(&s.dumpLeft, 0, 0) > 0 &&
                bp::mem::Plausible(static_cast<uintptr_t>(*raxSlot)) &&
                bp::mem::Readable(static_cast<uintptr_t>(*raxSlot), s.retObj))
            {
                char tag[96];
                const char* cls = bp::mem::RttiShort(static_cast<uintptr_t>(*raxSlot));
                snprintf(tag, sizeof tag, "%s returned object%s%s", s.name, cls ? " " : "", cls ? cls : "");
                bp::dump::Object(tag, static_cast<uintptr_t>(*raxSlot), s.retObj);
            }
        }
    ...
    // eErr name -> the global holding its value -> every instruction that reads
    // it -> the function each one is in. Writes those functions' addresses to
    // `out` and returns how many.
    //
    // More than one is normal and is not an error: a single error name can be
    // raised from several handlers, and eErrNoFieldMoveHolding is raised from
    // three on this build. Picking one of them would be a guess, so all of
    // them are returned and all of them get hooked.
    int ResolveBySymbol(const char* sym, uintptr_t* out, int max)
    {
        int leas = 0;
        const uintptr_t g = bp::resolve::ErrorGlobal(sym, &leas);
        if (!g) return 0;
        uintptr_t readers[8] = {};
        int total = 0;
        const int found = bp::resolve::Readers(g, readers, 8, &total);
        LOG("[resolve] %s: %d leas -> global +0x%llX (%s), read by %d instruction%s", sym, leas,
            static_cast<unsigned long long>(g), bp::resolve::SectionOf(g), total, total == 1 ? "" : "s");
        int n = 0;
        for (int i = 0; i < found && n < max; ++i)
        {
            const uintptr_t fn = bp::resolve::FunctionStart(readers[i]);
            if (!fn)
            {
                LOG_ERR("[resolve] %s: no unwind entry covers +0x%llX, so its function start is unknown",
                        sym, static_cast<unsigned long long>(readers[i]));
                continue;
            }
            // The same function can raise one error in several places; hooking
            // it twice would install two thunks on one entry.
            bool dup = false;
            for (int k = 0; k < n; ++k) if (out[k] == bp::mem::Game().base + fn) dup = true;
            if (dup) continue;
            LOG("[resolve]   reader +0x%llX in %s -> function +0x%llX",
                static_cast<unsigned long long>(readers[i]), bp::resolve::SectionOf(readers[i]),
                static_cast<unsigned long long>(fn));
            out[n++] = bp::mem::Game().base + fn;
        }
        return n;
    }
    ...
    bool ParseSite(const char* name, const char* spec, Site& out)
    {
            ...
            else
            {
                ...
                if (eq)
                {
                    *eq = 0;
                    ...
                    else if (!_stricmp(tok, "onerr")) out.onErr = v != 0;
                    else if (!_stricmp(tok, "onnull")) out.onNull = v != 0;
                    else if (!_stricmp(tok, "only"))
                    {
                        char* val = eq + 1;
                        Trim(val);
                        strncpy(out.only, val, sizeof out.only - 1);
                    }
                    else if (!_stricmp(tok, "errlines")) out.errLinesLeft = static_cast<LONG>(v);
                    else if (!_stricmp(tok, "mark"))
                    {
                        // Digits, each an argument to remember: mark=13 is the
                        // first and the third.
                        for (const char* q = eq + 1; *q; ++q)
                            if (*q >= '1' && *q <= '6') out.markArgs |= static_cast<uint8_t>(1u << (*q - '1'));
                    }
                    else if (!_stricmp(tok, "onlymarked")) out.onlyMarked = v != 0;
                    else if (!_stricmp(tok, "marks")) out.marksBytes = static_cast<unsigned>(v > 0x400 ? 0x400 : v);
        ...
        if (!rva) return false;
        out.target = bp::mem::Game().base + rva;
        return true;
    }
    ...
    // Everything that turns a filled-in Site into a live hook. The ini loop,
    // AddSite and the [resolve] loop all land here, so a site added from code
    // is hooked and logged exactly like one read out of the file.
    bool InstallSite(Site& s)
    {
        if (g_n >= kMaxSites) { LOG_ERR("[site] %s: no room, kMaxSites is %d", s.name, kMaxSites); return false; }
        if (g_shadowStack) s.leaveHook = false;
        if (!bp::mem::Executable(s.target, 16))
        {
            LOG_ERR("[site] %s: +0x%llX is not executable memory, not hooked", s.name,
                    static_cast<unsigned long long>(bp::mem::Rva(s.target)));
            return false;
        }
        // The entry thunk needs the trampoline address, and farhook needs the
        // detour address, so build the leave thunk first, then install with a
        // placeholder and patch the jump target.
        s.leave = MakeLeave(g_n);
        uint64_t* jumpSlot = nullptr;
        uint8_t* enter = MakeEnter(g_n, &jumpSlot);
        if (!s.leave || !enter) { LOG_ERR("[site] %s: thunk page allocation failed", s.name); return false; }
        char why[96] = "";
        if (!bp::farhook::Install(s.name, s.target, enter, &s.orig, why, sizeof why))
        {
            LOG_ERR("[site] %s at +0x%llX could not be hooked: %s", s.name,
                    static_cast<unsigned long long>(bp::mem::Rva(s.target)), why);
            return false;
        }
        // farhook published the trampoline before patching the entry, and the
        // entry patch is what makes the thunk reachable, so a game thread can
        // already be running through it. The jump slot is filled with an
        // interlocked store for that reason, and the window is the few
        // instructions between the patch and this line.
        InterlockedExchange64(reinterpret_cast<volatile LONG64*>(jumpSlot),
                              static_cast<LONG64>(reinterpret_cast<uint64_t>(s.orig)));
        s.enter = enter;
        LOG_OK("[site] %s hooked at +0x%llX%s%s%s%s (%s)", s.name,
               static_cast<unsigned long long>(bp::mem::Rva(s.target)),
               s.hasRet ? ", return value replaced" : "",
               s.hasSkip ? ", skipped entirely" : "",
               (s.hasRet || s.hasSkip) && s.after ? " after the first calls" : "",
               s.leaveHook ? "" : ", no return hook",
               s.onErr ? "silent unless it returns an error"
                        : (s.onNull ? "silent unless it returns nothing" : "per-call logging"));
        ++g_n;
        return true;
    }
...
namespace bp::sites
{
    ...
    bool Install(bool research)
    {
        ...
        // [resolve]: the same hooks, addressed by an eErr name instead of an
        // RVA. A game update moves every RVA in the ini and moves none of the
        // names, so this is the section that survives one.
        if (ReadSection(L"resolve", buf, 32768) > 0)
        {
            for (wchar_t* p = buf; *p && g_n < kMaxSites; p += wcslen(p) + 1)
            {
                wchar_t* eq = wcschr(p, L'=');
                ...
                char sym[96] = "";
                char opts[256] = "";
                if (const char* comma = strchr(spec, ','))
                {
                    const size_t k = static_cast<size_t>(comma - spec);
                    memcpy(sym, spec, k < sizeof sym - 1 ? k : sizeof sym - 1);
                    strncpy(opts, comma + 1, sizeof opts - 1);
                }
                else strncpy(sym, spec, sizeof sym - 1);
                Trim(sym); Trim(opts);
                uintptr_t fns[8] = {};
                const int nf = ResolveBySymbol(sym, fns, 8);
                if (!nf) { LOG_ERR("[site] %s: %s did not resolve to a function, not hooked", name, sym); continue; }
                for (int k = 0; k < nf && g_n < kMaxSites; ++k)
                {
                    // One function keeps the plain name; several are numbered,
                    // so a log line says which handler answered.
                    char one[64];
                    if (nf == 1) strncpy(one, name, sizeof one - 1), one[sizeof one - 1] = 0;
                    else snprintf(one, sizeof one, "%s%d", name, k + 1);
                    if (AddSite(one, fns[k], opts)) ++ok;
                }
            }
        }
        if (!g_n && !g_np && !g_nw)
    ...
    bool AddSite(const char* name, uintptr_t target, const char* spec)
    {
        ...
        // The room check used to sit in InstallSite, after `g_sites[g_n]` had
        // been written: with the table full that wrote one past the end, over
        // the count itself, and the game crashed five seconds in.
        if (g_n >= kMaxSites) { LOG_ERR("[site] %s: no room, kMaxSites is %d", name, kMaxSites); return false; }
        char full[320];
        snprintf(full, sizeof full, "0x%llX%s%s",
                 static_cast<unsigned long long>(mem::Rva(target)),
                 (spec && *spec) ? ", " : "", (spec && *spec) ? spec : "");
        Site& s = g_sites[g_n];
        s = Site();
        if (!ParseSite(name, full, s)) { LOG_ERR("[site] %s: could not build a spec from \"%s\"", name, full); return false; }
        return InstallSite(s);
    }
    ...
    uint64_t RealReturnAt(const uint64_t* slot)
    {
        ...
        if (!ts) return 0;
        for (int k = ts->depth - 1; k >= 0; --k)
            if (ts->e[k].slot == slot) return ts->e[k].ret;
        return 0;
    }
    ...
    bool Marked(uint64_t v)
    {
        ...
        for (int i = 0; i < kMaxMarks; ++i)
            if (static_cast<uint64_t>(InterlockedCompareExchange64(&g_marks[i], 0, 0)) == v) return true;
        return false;
    }

    void Mark(uint64_t v, const char* by)
    {
        // The first mark of a session is the bounty pickup, which is what
        // makes the run worth keeping whatever happens next.
        bp::Log::Keep("pickup");
        const uintptr_t p = static_cast<uintptr_t>(v);
        if (!mem::Plausible(p) || !mem::Readable(p, 16)) return;
        auto put = [&](uint64_t x, const char* what) {
            if (Marked(x)) return;
            for (int i = 0; i < kMaxMarks; ++i)
            {
                if (InterlockedCompareExchange64(&g_marks[i], static_cast<LONG64>(x), 0) != 0) continue;
                const char* cls = mem::RttiShort(static_cast<uintptr_t>(x));
                LOG("[site] %s marked %s0x%llX%s%s (slot %d)", by, what, static_cast<unsigned long long>(x),
                    cls ? " " : "", cls ? cls : "", i);
                return;
            }
            LOG("[site] %s: no room to mark 0x%llX; %d objects are marked already", by,
                static_cast<unsigned long long>(x), kMaxMarks);
        };
        put(v, "");
        uintptr_t owner = 0;
        if (mem::ReadPtr(p + 8, &owner) && mem::Readable(owner, 16) && mem::RttiShort(owner))
            put(owner, "its +8, ");
    }
    ...
    bool Interesting(uint64_t v)
    {
        ...
        uintptr_t q = 0;
        if (mem::ReadPtr(p + 8, &q) && Marked(q)) return true;      // a component of a marked actor
        uintptr_t first = 0;
        if (mem::ReadPtr(p, &first) && !mem::InImage(first) && mem::ReadPtr(first + 8, &q) && Marked(q))
            return true;                                              // a sub-object of such a component
        return false;
    }

    uint64_t MarkAt(int i)
    {
        if (i < 0 || i >= kMaxMarks) return 0;
        return static_cast<uint64_t>(InterlockedCompareExchange64(&g_marks[i], 0, 0));
    }

    void ClearMarks()
    {
        for (int i = 0; i < kMaxMarks; ++i) InterlockedExchange64(&g_marks[i], 0);
    }
    ...
    void Summarise()
    {
        for (int i = 0; i < g_n; ++i)
        {
            ...
            if (calls == s.reportedCalls &&
                InterlockedCompareExchange(&s.errors, 0, 0) == s.reportedErrors) continue;
            const LONG rets = InterlockedCompareExchange(&s.returns, 0, 0);
            const LONG ov = InterlockedCompareExchange(&s.overridden, 0, 0);
            const LONG errs = InterlockedCompareExchange(&s.errors, 0, 0);
            const uint32_t lastErr = static_cast<uint32_t>(InterlockedCompareExchange64(&s.lastErr, 0, 0));
            const char* en = bp::errors::Name(lastErr);
            char tail[128] = "";
            if (errs && s.onNull)
                snprintf(tail, sizeof tail, ", %ld of them returned nothing", errs);
            else if (errs)
                snprintf(tail, sizeof tail, ", %ld of them returned an error, last %lu%s%s", errs,
                         static_cast<unsigned long>(lastErr), en ? " = " : "", en ? en : "");
            LOG("[site] %-14s %ld calls (+%ld since last), %ld returns, last rax 0x%llX%s%ld%s",
                s.name, calls, calls - s.reportedCalls, rets,
                static_cast<unsigned long long>(InterlockedCompareExchange64(&s.lastRet, 0, 0)),
                s.hasRet ? ", replaced " : ", overrides ", ov, tail);
            s.reportedCalls = calls;
            s.reportedErrors = errs;
        }
    }
}
